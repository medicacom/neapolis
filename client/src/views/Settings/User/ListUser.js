...
function ListUser({onlineStatus}) {
  ...
  async function updateUser(id,etat) {
    const tx = db.transaction('users', 'readwrite');
      ...
      for await (const cursor of index.iterate(parseInt(id))) {
        ...
        switch(etat){
          case 0:objRole.etat = 1; notify(1, "Activer avec succes");break; 
          case 1:objRole.etat = 0; notify(1, "Désactiver avec succes");break;
          default:break;
        } 
  ...
  //storeUsers
  const storeUsers = useCallback(async (resUsers) => {
    const tx = db.transaction("users", "readwrite");
